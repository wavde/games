...
function render() {
  ...
  for (let r=0;r<H;r++) for (let c=0;c<W;c++) {
    ...
    let pressTimer;
    d.addEventListener('touchstart', e => { pressTimer = setTimeout(()=>{flag(r,c); pressTimer=null;}, 400); }, {passive:true});
    d.addEventListener('touchend', () => { if (pressTimer) { clearTimeout(pressTimer); pressTimer=null; } });
    el.appendChild(d);
  }
}
